...
//	}
	...
	public WebDriver decimal_place_exit_price(WebDriver driver) {
		values = exit_price.getText();
		Assert.assertTrue(values.contains("(est.)"));
//		Assert.assertTrue(exitPrice_decimalPlaces ==2);
		return driver;
	}
	...
	public WebDriver order_value_et(WebDriver driver, String order_final_vlaue) {
		Assert.assertTrue(total_order_value_sell_order_screen.getText().contains(order_final_vlaue));
		return driver;
	}
...
//		Assert.assertFalse(advice_reason_textbox);
	...
	public WebDriver validate_draft_order(WebDriver driver) throws InterruptedException {
		Thread.sleep(3000);
		Assert.assertTrue(draft_new_order.isDisplayed());
		return driver;
	}
